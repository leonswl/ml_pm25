...
def load_dataset_from_feature_store(
    ...
    with init_wandb_run(
        ...
        raw_data_at = wandb.Artifact(
            name="pm25_singapore_feature_view",
            type="feature_view",
    ...
    with init_wandb_run(
        ...
        run.use_artifact("pm25_singapore_feature_view:latest")

        y_train, y_test = prepare_data(data, fh=fh)
        ...
        for split in ["train", "test"]:
            ...
            split_metadata = {
                "timespan": [
                    split_y.index.get_level_values(0).min(),
                    split_y.index.get_level_values(0).max(),
                ],
                "dataset_size": len(split_y),
...
def prepare_data(
    ...
    """
    Function to structure the data for training:
