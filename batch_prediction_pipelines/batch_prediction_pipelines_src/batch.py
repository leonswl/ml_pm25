...
def predict(
    ...
    if model_version is None:
        train_metadata = utility.load_json("train_metadata.json")
        model_version = train_metadata["model_version"]
    if start_datetime is None or end_datetime is None:
        feature_pipeline_metadata = utility.load_json("feature_pipeline_metadata.json")
        # start_datetime = datetime.strptime(
        #     feature_pipeline_metadata["start_date"],
        #     # feature_pipeline_metadata["datetime_format"],
        # )
        # end_datetime = datetime.strptime(
        #     feature_pipeline_metadata["end_date"],
        #     # feature_pipeline_metadata["datetime_format"],
        # )
    ...
    X, y = load_batch_data.load_data_from_feature_store(
        fs,
    ...
    predictions = forecast(model, fh=fh)
    ...
    #     level="timestamp"
    # ).max()
    predictions_start_datetime = predictions.index.min()
    predictions_end_datetime = predictions.index.max()
    logger.info(
        f"Forecasted pm2.5 from {predictions_start_datetime} to {predictions_end_datetime}."
    )
...
def forecast(model, X: Optional[pd.DataFrame] = None, fh: int = 24):
    """
    ...
    Args:
        ...
        X (pd.DataFrame): Optional; Exogenous data with area, consumer_type, and datetime_utc as index.
...
def save_for_monitoring(predictions: pd.DataFrame, start_datetime: datetime):
    ...
    The predictions are stored in an indexed dataframe with the following index:
    - timestamp: The timestamp of the predictions, e.g. "2020-01-01 00:00:00" with a frequency of 1 hour.
    """

    bucket = utility.get_bucket()

    cached_predictions = utility.read_blob_from(
    ...
    predictions = predictions.loc[predictions.index >= pd.Period(start_datetime, freq="H")]
